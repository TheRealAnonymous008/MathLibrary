...
TEST(MatrixInit, InitWithInitializerList) {
	...
	ASSERT_TRUE(A.At({ 2U, 1U }) == 8);
	ASSERT_TRUE(A.At({ 1U, 3U }) == -1);
	ASSERT_TRUE(A.At({ 0U, 2U }) == 3);
}

TEST(DISABLED_MatrixEquality, Equality) {
	Matrix<int, 3, 4> A;
	Matrix<int, 3, 4> B;
...
TEST(DISABLED_MatrixInequality, Inequality) {
	Matrix<int, 3, 4> A;
	Matrix<int, 3, 4> B;
...
TEST(DISABLED_MatrixTranspose, Transpose1) {
	Matrix<int, 3, 4> A = {
		1, 2, 3, 0,
		4, 5, 6, -1,
		7, 8, 9, -2
